use super::dummy_component::{ComponentLink, GridLock};
use crate::components::placement::{PlaceComponentEvent, Size};
use crate::game::{
...
pub fn drag_v2(
    ...
    for dragging in drag_events.iter() {
        ...
        let (_, _, draggable, mut box_transform, _, _, gridlock, _, _) =
            match draggable_entity.get_mut(dragging.target()) {
                Ok(e) => e,
                Err(_) => {
                    continue;
                }
        ...
        if gridlock.is_some()
            && mouse_pos.x >= grid_bottom_left_corner.x
            && mouse_pos.y >= grid_bottom_left_corner.y
            && mouse_pos.x < grid_top_right_corner.x
            && mouse_pos.y < grid_top_right_corner.y
        {
            let gridlock = gridlock.unwrap(); // Weird way of doing it cuz I wanna put the above in one expression
            let grid_slot = ((mouse_pos - grid_bottom_left_corner) / GRID_CELL_SIZE as f32).floor()
                - gridlock.grab_part;
            box_transform.translation = (grid_bottom_left_corner
                + Vec2::new(
                    GRID_CELL_SIZE as f32 * grid_slot.x,
                    GRID_CELL_SIZE as f32 * grid_slot.y,
                ))
            .extend(z);
        } else {
    ...
    for end in drag_end_events.iter() {
        ...
        if let Some(component) = places {
            match gridlock {
                None => {
                    error!("Cannot place component without gridlock")
                }
                Some(g) => {
                    let pointer_entity = pointers.get_entity(end.pointer_id()).unwrap();
                    let pointer_location =
                        locations.get(pointer_entity).unwrap().location().unwrap();
                    let pointer_position = pointer_location.position;
                    let target = pointer_location
                        .target
                        .get_render_target_info(&windows, &images)
                    ...
                    let target_size = target.physical_size.as_vec2() / target.scale_factor as f32;
                    let mouse_pos = (pointer_position - (target_size / 2.0)) / pixel_zoom;
                    let grid_slot = ((mouse_pos - grid_bottom_left_corner) / GRID_CELL_SIZE as f32)
                        .floor()
                        - g.grab_part;
                    if grid_slot.x >= 0.0
                        && grid_slot.y >= 0.0
                        ...
                        && grid_slot.y < GRID_CELL_AMOUNT_HEIGHT as f32
                    {
                        ...
                        place_event_creator.send(PlaceComponentEvent(
                            [grid_slot.x as usize, grid_slot.y as usize],
                            component.0.clone(),
                        ));
                    }
                }
