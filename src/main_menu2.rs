use bevy::{prelude::{App, Plugin, in_state, Commands, Local, Image, Handle, FromWorld, AssetServer, World, IntoSystemConfigs, EventWriter, NextState}, window::close_on_esc, app::AppExit};
use bevy_egui::{EguiContexts};
use egui::{Frame, Color32, style::Margin, Rect as EguiRect, pos2, Align2, vec2, Vec2, RichText, Stroke};
use crate::{GameState};
...
fn main_menu(
    ...
    egui::CentralPanel::default().frame(Frame::default().outer_margin(Margin::same(0.0))).show(ctx_mut, |ui| {
        let uv = EguiRect::from_min_max(pos2(0.0, 0.0), pos2(1.0, 1.0));
        ui.painter().image(background, ui.available_rect_before_wrap(), uv, Color32::WHITE);
        egui::Area::new("buttons").anchor(Align2::CENTER_CENTER, vec2(0.0, 0.0)).show(ctx_mut, |ui| {
